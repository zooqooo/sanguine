...
describe('ActorStats getters', () => {
    ...
    it('getStatQuant', () => {
        let stats = new ActorStats([StatTypeEnum.Vigor, StatTypeEnum.Grit])
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[0])) //Add 4 Vigor, 2 Grit
        expect(stats.getStatQuant(StatTypeEnum.Vigor)).to.deep.equal(4)
    })
    ...
    it('getStatQuant (with damage type)', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.Protection, { baseType: DamageBaseTypeEnum.Fire, superType: DamageSuperTypeEnum.Stamina })).to.deep.equal(.1)
    })

    it('getStatQuant (without damage type)', () => {
        let stats = new ActorStats([StatTypeEnum.Protection])
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[25])) //+10% Fire Protection
        expect(() => stats.getStatQuant(StatTypeEnum.Protection)).toThrowError('is damage typed')
    })
...
describe('Bonuses should be applied according to the rules', () => {
    it('Add quant type should add values', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(3)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[2])) // Add 5 Tenacity
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(8)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[3])) // Subtract 4 Tenacity
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(4)
    })
    ...
    it('Increase quant type should multiply values with additive stacking', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(4.5)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[5])) // Increase Tenacity 200%
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(10.5)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[6])) // Decrease Tenacity 75% 
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(8.25)
    })
    ...
    it('More quant type should multiply values with multiplicitive stacking', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(13.5)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[9])) // 75% less Tenacity 
        expect(stats.getStatQuant(StatTypeEnum.Tenacity)).to.deep.equal(3.375)
    })
    ...
    it('Further quant type should geometrically combine values', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.TEST_FURTHER)).toBeCloseTo(.1,4)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[11])) // 30% Further TEST_FURTHER
        expect(stats.getStatQuant(StatTypeEnum.TEST_FURTHER)).toBeCloseTo(.37,4)
    })
    ...
    it('Stats with min or max defined should clamp final values', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.TEST_MIN_MAX_LINEAR)).to.deep.equal(1)
        ...
        expect(stats.getStatQuant(StatTypeEnum.TEST_MIN_MAX_LINEAR)).to.deep.equal(-2)
    })
})
...
describe('Armor should be applied properly', () => {
    it('Add test armor 1', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.Physical_Base_Armor)).to.deep.equal(100)
        expect(stats.getStatQuant(StatTypeEnum.Fire_Base_Armor)).to.deep.equal(0)
        expect(stats.getStatQuant(StatTypeEnum.Water_Base_Armor)).to.deep.equal(0)
    })
    ...
    it('Add test armor 2', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.Physical_Base_Armor)).to.deep.equal(100)
        expect(stats.getStatQuant(StatTypeEnum.Fire_Base_Armor)).toBeCloseTo(66.6666, 2)
        expect(stats.getStatQuant(StatTypeEnum.Water_Base_Armor)).to.deep.equal(0)
    })
    ...
    it('Add test armor 3', () => {
        ...
        expect(stats.getStatQuant(StatTypeEnum.Physical_Base_Armor)).to.deep.equal(100)
        expect(stats.getStatQuant(StatTypeEnum.Fire_Base_Armor)).toBeCloseTo(50, 2)
        expect(stats.getStatQuant(StatTypeEnum.Arcane_Base_Armor)).toBeCloseTo(90.91, 2)
        expect(stats.getStatQuant(StatTypeEnum.Water_Base_Armor)).to.deep.equal(0)
    })
})
...
describe('Static stats should be updated properly', () => {
    it('Check speed', () => {
        let stats = new ActorStats()
        expect(stats.getStatQuant(StatTypeEnum.Speed)).toBeCloseTo(0,2)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[22])) // +2 Agility, +2 Alacrity
        expect(stats.getStatQuant(StatTypeEnum.Speed)).toBeCloseTo(1.24,2)
        stats.addSource(new BonusSource(MOCK_BONUS_SOURCES[23])) // +5 Agility, -2 Alacrity
        expect(stats.getStatQuant(StatTypeEnum.Speed)).toBeCloseTo(1.83,2)
